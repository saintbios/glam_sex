...
<body>
    ...
    <div class="panel-container container galleries-panel">
        <h3>All photo galleries</h3>
        <div class="text-center pagination-list">
            <?php
                $nbPages = floor($_SESSION['nbActivePhotoGalleries']->nbActive/$_SESSION['itemsPerPage'])+1;
                for($i = 1; $i <= $nbPages; $i++) {
                    if($i == $activePage) {
                        ...
                        ?>
                       <a href="<?php echo site_url('/galleries/photos/' . $i);?>"><?php echo $i;?></a>
                        <?php
                        if($i != $nbPages) {
                            echo ' - ';
                        }
