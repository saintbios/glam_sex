...
class Country extends CI_Model
{
	...
	{
		if(! $query = $this->db->get_where('country', array('id' => $pId))) {
			return $this->db->error();
		} else {
			if ($query->num_rows() > 0) {
				return $query->row();
	...
	{
		if(! $query = $this->db->get_where('country', array('label' => $pLabel))) {
			return $this->db->error();
		} else {
			if ($query->num_rows() > 0) {
				return $query->row();
	...
	private function _insert($pLabel)
	{
		...
		if(! $this->db->insert('country', $insert_data)) {
			return $this->db->error();
		} else {
